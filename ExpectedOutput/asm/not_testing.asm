...
# Entered Stack
  ...
  sw $t0, 0($sp) # 
  lw $t0, 0($sp) # 
  sw $t0, 4($sp) # Assigning to a
  addiu $sp, $sp, 4 # deallocate solve math
  addiu $sp, $sp, -4 # Allocate space for symbol: v
...
L_CondEnd0: # 
  ...
  lw $t0, 0($sp) # 
seq $t0, $t0, $0 # 
  sw $t0, 0($sp) # store the negated value
  lw $t0, 0($sp) # 
  lw $t1, 4($sp) # 
  or $t0, $t1, $t0 # 
...
# Entered Stack
  # Call printf
  addiu $sp, $sp, -4 # Allocate space for rvalue
  addiu $sp, $sp, -4 # allocate space for instance of v
